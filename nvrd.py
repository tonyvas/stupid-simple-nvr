...
def setup_limit_checkers(recorders:dict, config:dict):
    ...
    for name, recorder in recorders.items():
        ...
        if RECORDER_MAX_AGE_KEY not in recorder_config or recorder_config[RECORDER_MAX_AGE_KEY] is None:
            max_age_sec = None
        else:
            max_age_sec = float(recorder_config[RECORDER_MAX_AGE_KEY]) * 3600
            
            if max_age_sec < 0:
                raise Exception(f'Max age cannot be negative!')

        if RECORDER_MAX_DISK_KEY not in recorder_config or recorder_config[RECORDER_MAX_DISK_KEY] is None:
            max_disk_bytes = None
        else:
            max_disk_bytes = float(recorder_config[RECORDER_MAX_DISK_KEY]) * 1e9
...
def start_limit_checkers(limit_checkers:list[LimitManager], stop_event:threading.Event, idle_event:threading.Event):
...
def setup(config:dict):
    ...
    # Needs to be a set-to-stop for the sleep to work
    ...
    limit_threads = create_threads([ lambda: start_limit_checkers(limit_checkers, stop_event, are_checkers_idle) ])
